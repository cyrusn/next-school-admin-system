...
export default function DisciplineForm() {
  ...
  const IS_DC_DOWN =
    settings.IS_DC_DOWN === 'true' || settings.IS_DC_DOWN === true
  ...
  const isFilterF6 =
    settings.IS_FILTER_F6 === 'true' || settings.IS_FILTER_F6 === true
  const isDCAdmin =
    ROLE_ENUM[ROLE] === ROLE_ENUM['DC_ADMIN'] || ROLE === ROLE_ENUM['DC_ADMIN']

  const classcodes = Object.keys(groupedStudents).filter((code) => {
    if (isFilterF6 && !isDCAdmin && code.startsWith('6')) return false
    return true
  })
  ...
  const isSubmitDisabled =
    isDisabled || (isFilterF6 && !isDCAdmin && hasF6Student)
