...
const Attendance = ({ attendanceSummary }) => {
  ...
  const getStudent = (classcodeAndNo) => {
    ...
    if (isFilterF6 && !isDCAdmin && classcode?.startsWith("6")) {
      return undefined;
    }
  ...
  const updateGrwthData = async () => {
    ...
    const filteredStudents = students.filter(({ regno, classcode }) => {
      if (isFilterF6 && !isDCAdmin && classcode?.startsWith("6")) {
        return false;
      }
      return regnos.includes(regno);
    });
  ...
  const isDisabled = (rows) => {
    ...
    if (isFilterF6 && !isDCAdmin) {
      const hasF6Student = modifiedRows.some(({ student }) =>
        student?.classcode?.startsWith("6"),
      );
      if (hasF6Student) return true;
    }
